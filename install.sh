...
elif grep -q "archive.ubuntu.com" /etc/apt/sources.list 2>/dev/null; then 
...
if command -v java &>/dev/null; then 
  ...
  if [[ "$JAVA_VER" -ge 8 ]]; then 
    ...
    sudo DEBIAN_FRONTEND=noninteractive apt-get install -y openjdk-21-jre-headless && success "Java 安装完成" 
  fi 
else 
  warn "Java 未安装，正在安装 openjdk-21-jre-headless..." 
  sudo DEBIAN_FRONTEND=noninteractive apt-get install -y openjdk-21-jre-headless && success "Java 安装完成" 
fi
...
echo -e "${CYAN}│${RESET} 常用模型：glm-5.1(推荐)  claude-opus-4-6  gpt-5.3-codex"
echo -e "${CYAN}└─────────────────────────────────────────┘${RESET}"
